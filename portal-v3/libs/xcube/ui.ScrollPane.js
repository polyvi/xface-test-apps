...
// INSERT POINT: OPTIONS
		...
		(function pubicIScrollMethods(){
			['resetPosition', 'disable', 'enable', 'refresh', 'scrollBy', 'scrollToElement', 'scrollTo', 'on', 'getComputedPosition', 'goToPage', 'next', 'prev']
			.forEach(function(item){
				_this[item] = function(){
					return _scroller[item].apply(_scroller, arguments);
				};
			});
            _this.getter("iscroll",function(){return _scroller});
		})();

        (function pubicIScrollProperties(){
            ...
            'hasVerticalScroll','maxScrollX','maxScrollY','scale','options','startX','startY','translateZ','wrapperHeight','wrapperWidth','x','y','wrapper','scroller']
                .forEach(function(item){
...
//                    e.preventDefault();		// This seems to break default Android browser
                ...
                if ( this.options.useTransition && this.isInTransition ) {
                    ...
                    if(Math.abs(pos.y) <= this.scrollerHeight){
...
// INSERT POINT: _end
            ...
            _resize: function () {
                ...
                if(that.wrapper.clientWidth<=0 || that.wrapper.clientHeight<=0)return;
        ...
        function _initCustomEvent(){
            ...
            _scroller.on("scrollEnd",function(){
                if(isStartFromBottom && _scroller.distY < -150){
                    _scroller._execEvent("reachBottom");
                }else if(isStartFromTop && _scroller.distY > 150){
                    _scroller._execEvent("reachTop");
                }
                _scroller._execEvent("release");
            });
            _scroller.on("scrollMove",function(){
                if(isStartFromBottom && _scroller.distY < -150){
                    _scroller._execEvent("snapUp");
                }else if(isStartFromTop && _scroller.distY > 150){
                    _scroller._execEvent("snapDown");
                }else{
                    _scroller._execEvent("release");
                }
            });
